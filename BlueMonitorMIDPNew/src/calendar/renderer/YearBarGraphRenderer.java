...
public class YearBarGraphRenderer implements ReadDBListener{
	//================================定数====================================//
	 ...
	 * @param h
	 */
	public YearBarGraphRenderer(int width, int height){				
		this.width = width;
		this.height = height;
	...
	private void drawBarGraph(DojaGraphics g,int glY){
		...
		if(selectedIndex < 0){
			...
			str = (selectedIndex+1)+"月:"+barData[selectedIndex]+"個";		
	...
	public void onCompleteReadDB(int[] data,int type,long time) {
		...
		if(type == ReadDBData.TYPE_YEAR_BP_AVE || 
			...
			String typeStr = "";
			switch (type) {
			case ReadNumOfDBData.TYPE_YEAR_ECG_NUM:
				typeStr = "心電図";
				break;
			case ReadNumOfDBData.TYPE_YEAR_PLS_NUM:
				typeStr = "脈波";
				break;
			case ReadNumOfDBData.TYPE_YEAR_BP_NUM:
				typeStr = "血圧";
				break;
			case ReadNumOfDBData.TYPE_YEAR_HR_NUM:
				typeStr = "心拍数";
				break;
			}// End of switch

			dateLabel = typeStr + " " + dateArray[0] + "年";
