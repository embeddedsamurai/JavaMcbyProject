...
public class WeekBarGraphRenderer implements ReadDBListener{
	//================================定数====================================//
	 ...
	 * @param h
	 */
	public WeekBarGraphRenderer(int width, int height){				
		this.width = width;
		this.height = height;
	...
	public int[] getSelectedDate(){
		if(selectedIndex ==-1){
			...
			return new int[]{dateArray[0],dateArray[1],weekDays[selectedIndex],0};
		}
	}
	...
	private void drawBarGraph(DojaGraphics g,int glY){
		...
		if(selectedIndex < 0){
			...
			for(int i = 0; i < barData.length; i++){
				...
				g.drawString(weekDays[i]+"",x,glY + FONT_ASCENT);
			}//End of for()
			...
			str = weekDays[selectedIndex]+"曜:"+barData[selectedIndex]+"個";		
	...
	public void onCompleteReadDB(int[] data,int type,long time) {
		...
		if(type == ReadDBData.TYPE_WEEK_BP_AVE || 
			     type == ReadDBData.TYPE_WEEK_HR_AVE ){
			//折れ線グラフ用データ
			polygonalFlag = true;
			
			// ７個＝一週間分のデータでなければ表示しない
			if (data.length != 7) {
				//-1のときはデータなしと表示される
				selectedIndex = -1;
				return;
			...
			String typeStr = "";
			switch (type) {
			case ReadNumOfDBData.TYPE_WEEK_ECG_NUM:
				typeStr = "心電図";
				break;
			case ReadNumOfDBData.TYPE_WEEK_PLS_NUM:
				typeStr = "脈波";
				break;		
			case ReadNumOfDBData.TYPE_WEEK_BP_NUM:
				typeStr = "血圧";
				break;
			case ReadNumOfDBData.TYPE_WEEK_HR_NUM:
				typeStr = "心拍数";
				break;
			}// End of switch

			int week = CalendarUtil.getCurrentWeekNum(dateArray[0],dateArray[1], dateArray[2]);
			
			dateLabel = typeStr + " " + dateArray[1] + "月 " + week + "週";
			System.out.println(dateLabel);

			// 日付ラベル
			weekDays = CalendarUtil.getWeekDays(dateArray[0], dateArray[1],week);
